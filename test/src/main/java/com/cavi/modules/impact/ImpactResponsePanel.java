...
public class ImpactResponsePanel extends JPanel {
    ...
    private void onEditRule() {
        JPanel p = new JPanel(new java.awt.GridLayout(3, 2, 8, 8));
        JComboBox<ThresholdRule.Op> opC = new JComboBox<>(ThresholdRule.Op.values());
        opC.setSelectedItem(currentRule.getOp());
        javax.swing.JTextField thr = new javax.swing.JTextField(Double.toString(currentRule.getThreshold()));
        javax.swing.JTextField windowSec = new javax.swing.JTextField("0");

        p.add(new JLabel("Operator:"));
        p.add(opC);
        p.add(new JLabel("Threshold:"));
        p.add(thr);
        p.add(new JLabel("Window (sec, 0=point):"));
        p.add(windowSec);

        int ok = javax.swing.JOptionPane.showConfirmDialog(this, p, "Edit Rule", javax.swing.JOptionPane.OK_CANCEL_OPTION);
